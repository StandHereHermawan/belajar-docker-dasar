...
--mount "type=volume,source=mongo-restore,destination=/data" \
ubuntu:jammy \
bash -c "cd /data && tar xvf /backup/backup-1.tar.gz --strip 1"

# Command membuat container dengan volume backup
docker container create \
--name mongo-restore \
--mount "type=volume,source=mongo-restore,destination=/data/db" \
--publish 27020:27017 \
